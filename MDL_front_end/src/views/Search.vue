  ...
  <div class="search">
        ...
        <v-flex xs12 >
          <v-layout align-center justify-start row wrap>
              <v-layout row wrap>
                ...
                <v-flex xs1>
                    <v-btn @click="show_visu" right fab flat class="ml-4 mt-4 mb-4 pink_custom" color="white"> 
    ...
    >
        ...
        <v-toolbar 
          ...
          <v-btn width="54px" height="54px" depressed class="grey_custom" @click="dialog = true" 
            :disabled="this.saved">
    ...
    >
      ...
      <v-card>
        ...
        <div style="padding-bottom: 15px">
            ...
            <a class="pink_custom--text">{{some_data.nodes[current_val].date}}</a>
            </div>
            <div style = "padding-left: 20%; max-width: 50%; display: inline">
                <a  class="pink_custom--text">{{some_data.nodes[current_val].user.lastname + " " + this.some_data.nodes[current_val].user.firstname + ' (' + this.some_data.nodes[current_val].user.establishment + ')'}} </a>
            </div>
        ...
        <v-card-actions>
          <v-btn
            ...
            :href="'/soa?id=' + this.some_data.nodes[current_val].id +'&tags=' +this.redirect_tags"
          >
            View
          </v-btn>
        </v-card-actions>
    ...
    //console.log(json);
      ...
      return node.title;
       ...
       let urlParams = new URLSearchParams(window.location.search);
        ...
        url: 'http://localhost:8080/UsersName/'+localStorage.getItem('accessUsername').toString()
      }).then(response => {
       ...
       var self = this;
        setTimeout(function(){
          axios({
              ...
              url: 'http://localhost:8080/saveResearch/keyword/'+ tags_list +'/user/'+ self.id_user.toString(),
              headers: {
  ...
  mounted: function () {
     let urlParams = new URLSearchParams(window.location.search);
      ...
      if(!(url_tags[0] =='' && url_tags.length==1)){
        ...
        url: 'http://localhost:8080/research/'+tags_str
        ...
        for (const key in response.data.soa) {
          console.log(response.data.soa[key].cluster);
        ...
        console.log(response.data.soa.sort());
        console.log(response.data.soa)
        ...
        /* Legend */ 
        var namesColors = []
        //id_cluster = id_cluster.sort()
          //console.log(id_cluster);
          /*
          var enter = false;
         for(var i=0; i < id_cluster.length; i++){
          //console.log(response.data.cluster[id_cluster[i]]);
            if(!enter){
              namesColors.push([colorsClusters[i], response.data.cluster[id_cluster[i]]]);
              }else{
                namesColors.push([colorsClusters[i+1], response.data.cluster[id_cluster[i]]]);
              }
          if(i ==0 && !enter){
            console.log("ici: "+i)
            id_cluster.shift()
    ...
    var val = 1;
       axios({
        method: 'GET',
        //
        url: 'http://localhost:8080/StateOfArts/'
      }).then(response => {
